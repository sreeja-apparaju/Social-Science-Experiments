* **************************************************************************** *
* *** Table S3: Relication Results (Stage 1) ***                               *
* **************************************************************************** *
quietly {
	clear all
	set more off
	...
	global   file_name "S3 - ReplicationResultsS1"
}
...
quietly {
	* keep variables for table
	...
	local   varlist "
			...
			r_rs1 p_rs1 n_rs1 in_rs1 space
			rep_sr_rs1 pow_rs1 rr_rs1
			";
	#delimit cr
	keep 	`varlist'

	* replace replication indicators
	tostring rep_sr_rs1, force replace
	replace  rep_sr_rs1 = "yes" if rep_sr_rs1 == "1"
	replace  rep_sr_rs1 = "no"  if rep_sr_rs1 == "0"

	* replace p-values by strings
	tostring p_os p_rs1, force format(%6.3f) gen(p_os_s p_rs1_s)
	replace  p_os_s  = "<0.001" if p_os  < 0.001
	replace  p_rs1_s = "<0.001" if p_rs1 < 0.001
	drop     p_os p_rs1
	rename   (p_os_s p_rs1_s) (p_os p_rs1)
	
	* add parantheses around number of individuals
	tostring in_os in_rs1, force replace
	replace  in_os  = ""                 if in_os  == "."
	replace  in_rs1 = ""                 if in_rs1 == "."
	replace  in_os  = "(" + in_os  + ")" if in_os  != ""
	replace  in_rs1 = "(" + in_rs1 + ")" if in_rs1 != ""
}
